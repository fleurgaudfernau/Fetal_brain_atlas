...
def create_dataset(template_spec, visit_ages=None, dataset_filenames=None, subject_ids=None, 
    ...
    if dataset_filenames is not None:
        for i, subject in enumerate(dataset_filenames): #for each subject
            ...
            for j, observations in enumerate(subject): #for each observation of i
                ...
                for (object_id, template_object) in template_spec.items(): #for each object of i eg right hippocampus
                    if object_id not in observations:
                        raise RuntimeError('The template object {} is not found for the visit {} of subject {}'\
                                            .format(object_id, j, i))
                    else:
                        object_list.append(DeformableObjectReader().create_object(
                                                                observations[object_id], 
                                                                template_object['object_type'], 
    ...
    longitudinal_dataset = LongitudinalDataset(subject_ids, visit_ages, deformable_objects = objects_dataset)
...
def create_scalar_dataset(group, observations, timepoints):
    ...
    longitudinal_dataset = LongitudinalDataset(subject_ids, times, scalars)
...
def create_image_dataset(group, observations, timepoints):
    ...
    longitudinal_dataset = LongitudinalDataset(subject_ids, times, images)
    longitudinal_dataset.number_of_subjects = len(subject_ids)
    longitudinal_dataset.total_number_of_observations = len(timepoints)
...
def read_and_create_image_dataset(dataset_filenames, visit_ages, subject_ids, template_spec):
    """
    ...
    for i, subject in enumerate(dataset_filenames):
        subject_objects = []
        for j, observations in enumerate(subject):
            for object_id in template_spec.keys():
                if object_id not in observations:
                    raise RuntimeError('The template object {} is not found for the visit {} of subject {}'\
                                            .format(object_id, j, i))
                else:
                    objectType = template_spec[object_id]['object_type']
                    reader = DeformableObjectReader()
                    deformable_object_visit = reader.create_object(observations[object_id], objectType)
                    deformable_object_visit.update()
            subject_objects.append(deformable_object_visit)
        if len(subject_objects) <= 1:
            warnings.warn("I have only one observation for subject {}".format(str(i)))
        objects_dataset.append(subject_objects)

    times = [np.array(elt) for elt in visit_ages]
    longitudinal_dataset = LongitudinalDataset(times, subject_ids, objects_dataset)
...
def create_template_metadata(template_spec, dimension=None, gpu_mode=None):
    """
    Creates a longitudinal dataset object from xml parameters.
    """
    if gpu_mode is None:
    ...
    for object_id, object in template_spec.items():
        filename = object['filename']
        object_type = object['object_type'].lower()

        assert object_type in ['SurfaceMesh'.lower(), 'PolyLine'.lower(), 'Landmark'.lower(),
        ...
        if object_norm in ['current', 'varifold']:
            objects_norm_kernels.append(kernel_factory.factory(
                                        gpu_mode=gpu_mode, kernel_width=object['kernel_width']))
        else:
    ...
    return objects_list, objects_name_extension, objects_noise_variance, multi_object_attachment
...
def compute_noise_dimension(template, multi_object_attachment, dimension):
    ...
    for k in range(len(template.object_list)):
        ...
        if multi_object_attachment.attachment_types[k] in ['current', 'varifold']:
            noise_dimension = 1
