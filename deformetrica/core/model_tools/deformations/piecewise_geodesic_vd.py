...
class Geodesic: #fg (before Geodesic)
    ...
    def __init__(self, dense_mode=default.dense_mode,
        ...
        self.num_component = num_components #num_components[i][j] = index for class i of component j
        #useless if no clustering
        ...
        self.tR = [None] * (self.nb_exponentials + 1)
    ...
    def get_template_points(self, time):
        """
        Returns the position of the landmark points deformed by the geodesic, at a given time.
        ...
        # Fetch index j of time closest (and above) to 'time'
    ...
    def parallel_transport(self, momenta_to_transport_tR, is_orthogonal=False):
        """
        :param momenta_to_transport_tR: the vector to parallel transport, given at tR and carried at control_points
    ...
    def get_control_pointstrajectory(self):
        if any(self.shoot_is_modified):
    ...
    def write(self, root_name, objects_name, objects_extension, template, template_data, output_dir,
        ...
        if write_adjoint_parameters:
            control_points_t = [elt.detach().cpu().numpy() for elt in self.get_control_pointstrajectory()]
