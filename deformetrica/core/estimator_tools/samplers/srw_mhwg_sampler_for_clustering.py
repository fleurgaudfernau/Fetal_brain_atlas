    ...
    def sample(self, statistical_model, dataset, population_RER, individual_RER, iteration, current_model_terms=None):
        ...
        for random_effect_name, proposal_RED in self.individual_proposal_distributions.items():
            ...
            for i in range(dataset.number_of_subjects):
                ...
                # Remember if the subject class was modified
                if random_effect_name == 'classes' and candidate_RER[i] == current_RER[i]: 
                    modified_subject[i] = 0
            ...
            # Tempering stage??
            #This is not tempering, this is just trying to get classes to changes
            print("Tempering?")
            print("tau before", tau)
            tau = np.array(tau)
            if random_effect_name == 'classes' and iteration % 10 < 5: #true during five iters then false during five iters 
                T = 1 - ((5 - (iteration % 10))/5) + 2*np.median(abs(tau[modified_subject != 0])) \
                        / (iteration // 20 + 1) * ((5 - (iteration % 10))/5)
            else: T = 1
