...
class LongitudinalAtlasSimplified(AbstractStatisticalModel):
    """
    ...
    def __init__(self, template_specifications,
        ...
        self.is_frozen = {'template_data': freeze_template, 'control_points': freeze_control_points,
                          ...
                          "acceleration" : True, "onset_age": True, "sources": False} #ajout fg

        # Prior distributions on the fixed effects
        ...
        # Individual random effects distributions
        ...
        if not self.is_frozen['acceleration']:
            self.individual_random_effects['acceleration'] = MultiScalarNormalDistribution()
        ...
        # Template.
        (object_list, self.objects_name, self.objects_name_extension, objects_noise_variance, self.multi_object_attachment) = \
        create_template_metadata(template_specifications, self.dimension, gpu_mode=self.gpu_mode)
        ...
        # Momenta.
        self.set_momenta(initialize_momenta(initial_momenta, self.number_of_control_points, self.dimension, random=True))
        self.__initialize_momenta_prior()

        # Modulation matrix.
        self.number_of_sources = number_of_sources
        self.fixed_effects['modulation_matrix'] = initialize_modulation_matrix(initial_modulation_matrix, self.number_of_control_points, self.number_of_sources, self.dimension)
        ...
        # Time-shift variance. #(if not self.is_frozen['onset_age'])
        ...
        assert self.number_of_sources is not None, 'Please specify the number of sources, or provide a modulation matrix file.'
        ...
        if not self.is_frozen['onset_age']:
            ...
            self.individual_random_effects['onset_age'].mean = 0
            self.individual_random_effects['onset_age'].variance_sqrt = 0
    ...
    def initialize_random_effects_realization(
            self, number_of_subjects, initial_sources=default.initial_sources,
            initial_onset_ages=default.initial_onset_ages, #default=None
            initial_accelerations=default.initial_accelerations, **kwargs):

        # Initialize the random effects realization.
        # These are used by the estimator function
        # Even if RE are frozen, they can still have a value

        individual_RER = dict()
        individual_RER['sources'] = initialize_sources(initial_sources, number_of_subjects, self.number_of_sources)
        individual_RER['onset_age'] = initialize_onset_ages(initial_onset_ages, number_of_subjects, self.get_reference_time())
        individual_RER['acceleration'] = initialize_accelerations(initial_accelerations, number_of_subjects)
    ...
    def compute_log_likelihood(self, dataset, population_RER, individual_RER, mode='complete', with_grad=False,
        ...
        if mode in ['complete', 'class2']:
            regularity += self._compute_class2_priors_regularity(template_data, momenta, modulation_matrix)
        ...
        if with_grad:
            ...
            if mode == 'complete': # When performing GD/deterministic estimation...
                if not self.is_frozen['sources']:
    ...
    def compute_sufficient_statistics(self, dataset, population_RER, individual_RER, residuals=None, model_terms=None):
        ...
        # Second statistical moment of the residuals (most costly part).
            ...
            for iteration in range(max_number_of_iterations):
                ...
                if iteration == max_number_of_iterations:
                    msg = 'When updating the acceleration std parameter from the empirical std, the fixed-point ' \
                          ...
                          + str(max_number_of_iterations) + 'iterations. Difference = ' + str(difference) + ' > tolerance = ' + str(convergence_tolerance)
                    warnings.warn(msg)
    ...
    def _compute_class1_priors_regularity(self):
        ...
        if not self.is_frozen['time_shift_variance']:
            regularity += self.priors['time_shift_variance'].compute_log_likelihood(self.fixed_effects['time_shift_variance'])

        # Log-acceleration variance prior (if not frozen).
        if not self.is_frozen['acceleration_variance']:
            regularity += self.priors['acceleration_variance'].compute_log_likelihood(self.fixed_effects['acceleration_variance'])
    ...
    def _compute_class2_priors_regularity(self, template_data, momenta, modulation_matrix):
        """
        Fully torch.
    ...
    def _compute_residuals(self, dataset, template_data, absolute_times, sources, with_grad=True):
        ...
        # fg: erase parallel computations
        ...
        for i in range(len(targets)):
            ...
            for j, (absolute_time, target) in enumerate(zip(absolute_times[i], targets[i])):
                ...
                #get the template points of the average trajectory at time=absolute_time
                #and modify the exponential of spatiotemporal_reference_frame accordingly
                #its initial template points, cp are that at time=absolute time and  momenta = space_shift
                deformed_points = self.spatiotemporal_reference_frame.get_template_points(
                    absolute_time, sources[i], device=device)
                
                #deform the template according to deformed_points
                deformed_data = self.template.get_deformed_data(deformed_points, template_data)

                #distance between deformed template and target image
    ...
    def _compute_absolute_times(self, times, onset_ages, accelerations):
        """
        Fully torch.
        Compute time reparametrization for each subject

        times: list of arrays: observation times of each subject
        onset_ages: a tensor that lists the onset age of each subject
        """
        # Absolute times can be computed even if acc or osnet are frozen...
        ...
        #modif fg
        # if self.is_frozen['onset_age'] and self.is_frozen["acceleration"]:
        #     absolute_times = []
    ...
    def print(self, individual_RER):
        ...
        if self.is_frozen['onset_age']:
            logger.info('\t\t onset_ages is frozen')
        if self.is_frozen['acceleration']:
            logger.info('\t\t accelerations is frozen')
        if self.is_frozen['sources']:
            logger.info('\t\t sources is frozen')
        ...
        for i, subject_id in enumerate(dataset.subject_ids):
            ...
            for j, (time, absolute_time) in enumerate(zip(dataset.times[i], absolute_times[i])):
                ...
                if compute_residuals:
                    residuals_i.append(self.multi_object_attachment.compute_distances(deformed_data, self.template, targets[i][j]))
    ...
    def _write_model_parameters(self, individual_RER, output_dir):
        ...
        if not self.is_frozen["acceleration_variance"]:
            write_2D_array(np.zeros((1,)) + math.sqrt(self.get_acceleration_variance()), output_dir, self.name + "__EstimatedParameters__AccelerationStd.txt")
        if not self.is_frozen["noise_variance"]:
        ...
        if self.is_frozen["sources"]:
            write_2D_array(individual_RER['sources'], output_dir, self.name + "__FrozenParameters__Sources.txt")
        else:
            write_2D_array(individual_RER['sources'], output_dir, self.name + "__EstimatedParameters__Sources.txt")
        if self.is_frozen["onset_age"]:
            write_2D_array(individual_RER['onset_age'], output_dir, self.name + "__FrozenParameters__OnsetAges.txt")
        else:
            write_2D_array(individual_RER['onset_age'], output_dir, self.name + "__EstimatedParameters__OnsetAges.txt")
        if self.is_frozen["acceleration"]:
            write_2D_array(individual_RER['acceleration'], output_dir, self.name + "__FrozenParameters__Accelerations.txt")
        else:
            write_2D_array(individual_RER['acceleration'], output_dir, self.name + "__EstimatedParameters__Accelerations.txt")
...
################################################################################################################
    ...
    def compute_errors(self, previous_fixed_effects):
        """
            Save error rate (ajout fg)
        """
        error_rate = 0
        den = 0
        print(self.fixed_effects)
        (template_data, template_points, control_points, momenta, modulation_matrix) = self._fixed_effects_to_torch_tensors(False)

        for key in self.fixed_effects.keys():
            if key == "template_data":
                print(type(self.fixed_effects[key]["image_intensities"]))
                error_rate += np.sum(np.abs(self.fixed_effects[key]["image_intensities"] \
                                                - previous_fixed_effects[key]["image_intensities"]))
                den +=  np.sum(np.abs(previous_fixed_effects[key]["image_intensities"]))
            else:
                if not self.is_frozen[key]:
                    error_rate += np.sum(np.abs(self.fixed_effects[key] - previous_fixed_effects[key]))
                    den +=  np.sum(np.abs(previous_fixed_effects[key]))
        error_rate /= den
