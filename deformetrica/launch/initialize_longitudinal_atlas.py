...
def estimate_geodesic_regression_single_point(deformetrica, xml_parameters, regressions_output_path,
                                 global_dataset_filenames, global_visit_ages, global_subject_ids,
                                 global_tmin, global_initial_objects_template_path):
    # Create folder.
    regression_output_path = join(regressions_output_path, 'GeodesicRegression')
    if os.path.isdir(regression_output_path): shutil.rmtree(regression_output_path)
    ...
    print("xml_parameters.template_specifications", xml_parameters.template_specifications)
    print("global_initial_objects_template_path", global_initial_objects_template_path)
    ...
    xml_parameters.max_iterations = 1000
...
def estimate_subject_geodesic_regression(
        i, deformetrica, xml_parameters, regressions_output_path,
        global_dataset_filenames, global_visit_ages, global_subject_ids):
    logger.info('')
    logger.info('[ geodesic regression for subject ' + global_subject_ids[i] + ' ]')
    logger.info('')

    # Create folder.
    subject_regression_output_path = join(regressions_output_path,
                                                  'GeodesicRegression__subject_' + global_subject_ids[i])
...
def initialize_template_image_with_atlas(preprocessings_folder, xml_parameters, global_deformetrica,
                                        global_initial_cp_are_given, global_path_to_data, 
                                        model_xml_path, estimated_cp_path, atlas_output_path):
    """
    ...
    xml_parameters.max_iterations = 1000 ## TODO 
    ...
    global_initial_cp = model.get_control_points()

    global_initial_objects_template_path = []
    global_initial_objects_template_type = []
    for k, (object_name, obj_extension, original_obj_noise_var) \
        ...
        estimated_template_path = join(atlas_output_path, 'BayesianAtlas__EstimatedParameters__Template_' + object_name + obj_extension)
        global_initial_objects_template_path.append(join(global_path_to_data, 'ForInitialization__Template_' + object_name + '__FromAtlas' + obj_extension))
    ...
    if global_initial_cp_are_given:
        global_initial_cp_path = xml_parameters.initial_control_points
        global_initial_cp = read_2D_array(global_initial_cp_path)
    else:
        global_initial_cp_path = join(global_path_to_data, 'ForInitialization__ControlPoints__FromAtlas.txt')
        shutil.copyfile(estimated_cp_path, global_initial_cp_path)

    # Modify and write the model.xml file accordingly.
    model_xml_0 = et.parse(model_xml_path).getroot()
    model_xml_0 = insert_model_xml_template(model_xml_0, 'filename', global_initial_objects_template_path)
    model_xml_0 = insert_model_xml_template(model_xml_0, 'noise-std', global_objects_noise_std_string)
    model_xml_0 = insert_model_xml_level1_entry(model_xml_0, 'initial-control-points', global_initial_cp_path)
    model_xml_path = join(os.path.dirname(preprocessings_folder), 'initialized_model.xml')
    doc = parseString((et.tostring(model_xml_0).decode('utf-8').replace('\n', '').replace('\t', ''))).toprettyxml()
    np.savetxt(model_xml_path, [doc], fmt='%s')

    return global_initial_template_data, global_initial_cp, global_initial_template, \
            atlas_output_path, global_atlas_momenta, global_initial_objects_template_path, global_initial_objects_template_type, \
            global_initial_cp_path

def initialize_longitudinal_atlas_single_point(model_xml_path, dataset_xml_path, optimization_parameters_xml_path,
    ...
    global_path_to_data = join(os.path.dirname(preprocessings_folder), 'data')
    ...
    global_dataset_filenames = xml_parameters.dataset_filenames
    global_visit_ages = xml_parameters.visit_ages
    global_subject_ids = xml_parameters.subject_ids

    global_objects_name, global_objects_name_extension = create_template_metadata(xml_parameters.template_specifications)[1:3]

    global_dense_mode = xml_parameters.dense_mode
    global_kernel_type = xml_parameters.deformation_kernel_type
    global_kernel_width = xml_parameters.deformation_kernel_width
    global_kernel_device = xml_parameters.deformation_kernel_device

    global_nb_of_subjects = len(global_dataset_filenames)
    global_total_nb_of_observations = sum([len(elt) for elt in global_visit_ages])

    global_initial_cp_are_given = xml_parameters.initial_control_points is not None
    global_initial_momenta_are_given = xml_parameters.initial_momenta is not None
    global_initial_mod_matrix_is_given = xml_parameters.initial_modulation_matrix is not None
    global_initial_t0_is_given = xml_parameters.t0 is not None

    global_t0 = xml_parameters.t0
    #global_tmin = global_t0 if the dataset is single point...
    if not global_initial_t0_is_given:
        global_t0 = sum([sum(elt) for elt in global_visit_ages]) / float(global_total_nb_of_observations)

    global_tmin = sum([elt[0] for elt in global_visit_ages]) / float(global_nb_of_subjects)
    global_nb_of_tp = xml_parameters.number_of_time_points
    
    global_tensor_scalar_type = model_options['tensor_scalar_type']
    

    """ 
    0- Classify the type of dataset
    """

    dataset_type = "single_points"
    nb_of_longitudinal_subjects = 0
    longitudinal_subjects_indices = []
    #global_dataset_filenames: a list of n_subject lists containing disctionaries (1 dic/observation)
    print(global_dataset_filenames)

    for i in range(len(global_dataset_filenames)):
        if len(global_dataset_filenames[i]) > 1:
            nb_of_longitudinal_subjects += 1
            longitudinal_subjects_indices.append(i)
    
    if nb_of_longitudinal_subjects > 0:
        dataset_type = "mixed"
    ...
    Path to the outputs
    """
    atlas_output_path = join(preprocessings_folder, '1_atlas_on_baseline_data')
    regressions_output_path = join(preprocessings_folder, '2_individual_geodesic_regressions')
    regression_tmp_path = join(regressions_output_path, 'tmp')
    shooting_output_path = join(preprocessings_folder, '3_shooting_from_baseline_to_average')
    registration_output_path = join(preprocessings_folder, '4_longitudinal_registration')

    for path in [regressions_output_path, regression_tmp_path, shooting_output_path, registration_output_path]:
        if os.path.isdir(path): shutil.rmtree(path)
        os.mkdir(path)
    
    global_initial_momenta_path = join(global_path_to_data, 'ForInitialization__Momenta__FromRegressions.txt')
    estimated_cp_path = join(atlas_output_path, 'BayesianAtlas__EstimatedParameters__ControlPoints.txt')
    heuristic_onset_ages_path = join(global_path_to_data, 'ForInitialization__OnsetAges__FromHeuristic.txt')
    heuristic_accelerations_path = join(global_path_to_data, 'ForInitialization__Accelerations__FromHeuristic.txt')
    global_initial_mod_matrix_path = join(global_path_to_data, 'ForInitialization__ModulationMatrix__FromICA.txt')
    global_initial_sources_path = join(global_path_to_data, 'ForInitialization__Sources__FromICA.txt')

    estimated_onset_ages_path = join(registration_output_path, 'LongitudinalRegistration__EstimatedParameters__OnsetAges.txt')
    estimated_accelerations_path = join(registration_output_path, 'LongitudinalRegistration__EstimatedParameters__Accelerations.txt')
    estimated_sources_path = join(registration_output_path, 'LongitudinalRegistration__EstimatedParameters__Sources.txt')

    if dataset_type == "single_points":
        regressions_output_path = join(preprocessings_folder, '2_global_geodesic_regression')
    
    transported_regression_momenta_path = [join(regressions_output_path, 
                                            'GeodesicRegression__subject_' + global_subject_ids[i], 'GeodesicRegression__EstimatedParameters__TransportedMomenta.txt')\
                                            for i in range(len(global_subject_ids))]
    path_to_regression_cp = [join(regression_tmp_path, 'regression_cp__%s.txt' % global_subject_ids[i]) \
                            for i in range(len(global_subject_ids))]
    

    global_initial_template_data, global_initial_cp, global_initial_template, atlas_output_path, \
    global_atlas_momenta, global_initial_objects_template_path, global_initial_objects_template_type, \
    global_initial_cp_path, estimated_template_path \
    = initialize_template_image_with_atlas(preprocessings_folder, xml_parameters, global_deformetrica,
                                            global_initial_cp_are_given, global_path_to_data, model_xml_path, estimated_cp_path,
                                            atlas_output_path)
    ...
    if not global_overwrite and os.path.isdir(regressions_output_path):
        global_initial_momenta = read_3D_array(join(global_path_to_data, 'ForInitialization__Momenta__FromRegressions.txt'))
    
    # Check if an initial (longitudinal) momenta is available.
    elif global_initial_cp_are_given and global_initial_momenta_are_given:
        global_initial_momenta = read_3D_array(xml_parameters.initial_momenta)

    else:
        global_initial_momenta = np.zeros(global_initial_cp.shape)
        ...
        if dataset_type == "single_points":
            regression_cp, global_initial_momenta = estimate_geodesic_regression_single_point(
                                                            global_deformetrica, xml_parameters, regressions_output_path,
                                                            global_dataset_filenames, global_visit_ages, global_subject_ids,
                                                            global_tmin, global_initial_objects_template_path)
        else:
            # Regression for subjects with longitudinal observations
            for i in range(global_nb_of_subjects):
                if i in longitudinal_subjects_indices:

                    # Set the initial template as the reconstructed object after the atlas.
                    for k, (object_id, object_specs) in enumerate(xml_parameters.template_specifications.items()):
                        object_specs['filename'] = join(atlas_output_path, '%sAtlas__Reconstruction__%s__subject_%s%s' % ('Bayesian', object_id, global_subject_ids[i], global_objects_name_extension[k]))

                    # Find the control points and momenta that transforms the previously computed template into the individual.
                    registration_cp, registration_momenta = shoot(global_initial_cp, global_atlas_momenta[i],
                        global_kernel_width, global_kernel_type, number_of_time_points=global_nb_of_tp, 
                    ...
                    np.savetxt(path_to_regression_cp[i], registration_cp)
                    xml_parameters.initial_control_points = path_to_regression_cp[i]
                    ...
                    _, transported_regression_momenta = parallel_transport(
                        regression_cp, regression_momenta, - registration_momenta, global_kernel_width, 
                        global_kernel_type, global_kernel_device, number_of_time_points=global_nb_of_tp,
                        dense_mode=global_dense_mode, tensor_scalar_type=global_tensor_scalar_type)
                    ...
                    np.savetxt(transported_regression_momenta_path[i], transported_regression_momenta)

            # Divide to obtain the average momenta. Write the result in the data folder.
            global_initial_momenta /= float(nb_of_longitudinal_subjects)
        ...
        # Modify and write the model.xml file accordingly.
        model_xml_0 = et.parse(model_xml_path).getroot()
        model_xml_0 = insert_model_xml_level1_entry(model_xml_0, 'initial-momenta', global_initial_momenta_path)
        model_xml_path = join(os.path.dirname(preprocessings_folder), 'initialized_model.xml')
        doc = parseString((et.tostring(model_xml_0).decode('utf-8').replace('\n', '').replace('\t', ''))).toprettyxml()
        np.savetxt(model_xml_path, [doc], fmt='%s')
    ...
    global_initial_cp_torch = torch.from_numpy(global_initial_cp).type(global_tensor_scalar_type)
    ...
    for i in range(global_nb_of_subjects):
        if i in longitudinal_subjects_indices:
            ...
            subject_mean_observation_age = np.mean(np.array(global_visit_ages[i]))
            heuristic_onset_ages.append(subject_mean_observation_age)

            # Heuristic for the initial acceleration.
            subject_regression_momenta = read_3D_array(transported_regression_momenta_path[i])
            regression_momenta_torch = torch.from_numpy(subject_regression_momenta).type(global_tensor_scalar_type)

            momenta_sc_product_with_pop_momenta = torch.dot(
                global_initial_momenta_torch.view(-1), kernel.convolve(
                global_initial_cp_torch, global_initial_cp_torch, regression_momenta_torch).view(-1)).detach().cpu().numpy()

            if momenta_sc_product_with_pop_momenta <= 0.0:
                msg = 'Subject %s seems to evolve against the population: scalar_product = %.3E.' % \
                    (global_subject_ids[i], Decimal(float(momenta_sc_product_with_pop_momenta)))
                logger.info('>> ' + msg)
                heuristic_accelerations.append(1.0)  # Neutral initialization.
            else:
                heuristic_accelerations.append(float(np.sqrt(momenta_sc_product_with_pop_momenta / global_initial_momenta_norm_squared)))
        else:
            heuristic_onset_ages.append(np.array(global_t0))
    ...
    if dataset_type == "mixed":
        heuristic_onset_ages_longitudinal = np.take(heuristic_onset_ages, longitudinal_subjects_indices)
        heuristic_accelerations_longitudinal = np.take(heuristic_accelerations, longitudinal_subjects_indices)
        
        heuristic_time_shift_std = np.std(heuristic_onset_ages_longitudinal)
        heuristic_acceleration_std = get_acceleration_std(heuristic_accelerations_longitudinal)
    
        # Rescaling the initial momenta according to the mean of the acceleration factors.
        expected_mean_acceleration = float(truncnorm.stats(- 1.0 / heuristic_acceleration_std, float('inf'),
                                                        loc=1.0, scale=heuristic_acceleration_std, moments='m'))
        mean_acceleration = np.mean(heuristic_accelerations_longitudinal)
        heuristic_accelerations *= expected_mean_acceleration / mean_acceleration
        global_initial_momenta *= mean_acceleration / expected_mean_acceleration

        print("heuristic_accelerations rescaled", heuristic_accelerations)

        # Acceleration standard deviation, after whitening.
        heuristic_acceleration_std = get_acceleration_std(heuristic_accelerations)
    else:
        heuristic_time_shift_std = 0
        heuristic_acceleration_std = 0
            
    logger.info('>> Estimated fixed effects:')
    logger.info('\t\t time_shift_std    =\t%.3f' % heuristic_time_shift_std)
    logger.info('\t\t acceleration_std  =\t%.3f' % heuristic_acceleration_std)

    logger.info('>> Estimated random effect statistics:')
    logger.info('\t\t onset_ages    =\t%.3f\t[ mean ]\t+/-\t%.4f\t[std]' %
                (np.mean(heuristic_onset_ages), heuristic_time_shift_std))
    logger.info('\t\t accelerations =\t%.4f\t[ mean ]\t+/-\t%.4f\t[std]' %
                (np.mean(heuristic_accelerations), np.std(heuristic_accelerations)))

    # Export the results -----------------------------------------------------------------------------------------------
    global_initial_momenta_path = join(global_path_to_data, 'ForInitialization__Momenta__RescaledWithHeuristics.txt')
    
    np.savetxt(global_initial_momenta_path, global_initial_momenta)
    np.savetxt(heuristic_onset_ages_path, heuristic_onset_ages)    
    np.savetxt(heuristic_accelerations_path, heuristic_accelerations)

    # Modify the original model.xml file accordingly.
    model_xml_0 = et.parse(model_xml_path).getroot()
    model_xml_0 = insert_model_xml_level1_entry(model_xml_0, 'initial-momenta', global_initial_momenta_path)
    if heuristic_time_shift_std > 0.0:
        model_xml_0 = insert_model_xml_level1_entry(model_xml_0, 'initial-time-shift-std', '%.4f' % heuristic_time_shift_std)
    if heuristic_acceleration_std > 0.0:
        model_xml_0 = insert_model_xml_level1_entry(model_xml_0, 'initial-acceleration-std', '%.4f' % heuristic_acceleration_std)
    model_xml_0 = insert_model_xml_level1_entry(
        model_xml_0, 'initial-onset-ages', heuristic_onset_ages_path)
    model_xml_0 = insert_model_xml_level1_entry(
        model_xml_0, 'initial-accelerations', heuristic_accelerations_path)
    model_xml_path = join(os.path.dirname(preprocessings_folder), 'initialized_model.xml')
    doc = parseString((et.tostring(
        model_xml_0).decode('utf-8').replace('\n', '').replace('\t', ''))).toprettyxml()
    np.savetxt(model_xml_path, [doc], fmt='%s')
    ...
    if not global_initial_cp_are_given and not global_initial_momenta_are_given \
        and global_t0 != global_tmin: #global_t0 = global_tmin when dataset is single point
        ...
        if global_dense_mode:
            ...
            geodesic.set_control_points_t0(Variable(torch.from_numpy(global_initial_cp).type(global_tensor_scalar_type)))
        ...
        # Export results -----------------------------------------------------------------------------------------------
        number_of_timepoints = geodesic.forward_exponential.number_of_time_points + geodesic.backward_exponential.number_of_time_points - 2
    
        # Template.
        for k, (object_name, obj_extension) in enumerate(
                zip(global_objects_name, global_objects_name_extension)):
            # Copy the estimated template to the data folder.
            shooted_template_path = join(
                shooting_output_path, 'Shooting__GeodesicFlow__' + object_name + '__tp_' + str(number_of_timepoints)
                                        + ('__age_%.2f' % global_t0) + obj_extension)
            global_initial_objects_template_path[k] = join(
                global_path_to_data, 'ForInitialization__Template_' + object_name + '__FromAtlasAndShooting' + obj_extension)
        ...
        # Control points.
        shooted_cp_path = join(shooting_output_path, 'Shooting__GeodesicFlow__ControlPoints__tp_' + str(number_of_timepoints)
                                    + ('__age_%.2f' % global_t0) + '.txt')
        global_initial_cp_path = join(global_path_to_data, 'ForInitialization__ControlPoints__FromAtlasAndShooting.txt')
        shutil.copyfile(shooted_cp_path, global_initial_cp_path)

        # Momenta.
        shooted_momenta_path = join(shooting_output_path, 'Shooting__GeodesicFlow__Momenta__tp_' + str(number_of_timepoints)
                                    + ('__age_%.2f' % global_t0) + '.txt')
        global_initial_momenta_path = join(global_path_to_data, 'ForInitialization__Momenta__FromRegressionsAndShooting.txt')
    ...
    # Check if an initial (longitudinal) momenta is available.
    if not (global_initial_cp_are_given and global_initial_mod_matrix_is_given):
        ...
        momenta = read_3D_array(join(atlas_output_path, 'Bayesian' + 'Atlas__EstimatedParameters__Momenta.txt'))
        ...
        if xml_parameters.number_of_sources is not None:
            ...
            number_of_sources = 4
            logger.info('>> No initial modulation matrix given, neither a number of sources. The latter will be ARBITRARILY defaulted to 4.')
        ...
        # Save.
        np.savetxt(global_initial_mod_matrix_path, global_initial_mod_matrix)
        np.savetxt(global_initial_sources_path, global_initial_sources)

        # Modify the original model.xml file accordingly.
        model_xml_0 = et.parse(model_xml_path).getroot()
        model_xml_0 = insert_model_xml_level1_entry(model_xml_0, 'initial-modulation-matrix', global_initial_mod_matrix_path)
        model_xml_0 = insert_model_xml_level1_entry(model_xml_0, 'initial-sources', global_initial_sources_path)
        model_xml_path = join(os.path.dirname(preprocessings_folder), 'initialized_model.xml')
        doc = parseString((et.tostring(model_xml_0).decode('utf-8').replace('\n', '').replace('\t', ''))).toprettyxml()
        np.savetxt(model_xml_path, [doc], fmt='%s')

        logger.info('>> Estimated random effect statistics:')
        logger.info('\t\t sources =\t%.3f\t[ mean ]\t+/-\t%.4f\t[std]' % (np.mean(global_initial_sources), np.std(global_initial_sources)))
    ...
    global_acceleration_std = get_acceleration_std(global_accelerations)
    ...
    logger.info('>> Estimated random effect statistics:')
    logger.info('\t\t onset_ages    =\t%.3f\t[ mean ]\t+/-\t%.4f\t[std]' % (np.mean(global_onset_ages), global_time_shift_std))
    logger.info('\t\t accelerations =\t%.4f\t[ mean ]\t+/-\t%.4f\t[std]' % (np.mean(global_accelerations), np.std(global_accelerations)))
    logger.info('\t\t sources       =\t%.4f\t[ mean ]\t+/-\t%.4f\t[std]' % (np.mean(global_sources), np.std(global_sources)))

    # Copy the output individual effects into the data folder.
    # Initial momenta.
    global_initial_momenta_path = join(global_path_to_data, 'ForInitialization__Momenta__RescaledWithLongitudinalRegistration.txt')
    np.savetxt(global_initial_momenta_path, global_initial_momenta)

    # Onset ages.
    global_initial_onset_ages_path = join(global_path_to_data, 'ForInitialization__OnsetAges__FromLongitudinalRegistration.txt')
    shutil.copyfile(estimated_onset_ages_path, global_initial_onset_ages_path)

    # Accelerations.
    global_initial_accelerations_path = join(global_path_to_data, 'ForInitialization__Accelerations__FromLongitudinalRegistration.txt')
    np.savetxt(global_initial_accelerations_path, global_accelerations)

    # Sources.
    global_initial_sources_path = join(global_path_to_data, 'ForInitialization__Sources__FromLongitudinalRegistration.txt')
    ...
    if global_time_shift_std > 0:
        model_xml_0 = insert_model_xml_level1_entry(model_xml_0, 'initial-time-shift-std', '%.4f' % global_time_shift_std)
    if global_acceleration_std > 0:
        model_xml_0 = insert_model_xml_level1_entry(model_xml_0, 'initial-acceleration-std', '%.4f' % global_acceleration_std)
    model_xml_0 = insert_model_xml_level1_entry(
        model_xml_0, 'initial-onset-ages', global_initial_onset_ages_path)
    model_xml_0 = insert_model_xml_level1_entry(
        model_xml_0, 'initial-accelerations', global_initial_accelerations_path)
    model_xml_0 = insert_model_xml_level1_entry(
        model_xml_0, 'initial-sources', global_initial_sources_path)
    model_xml_path = join(os.path.dirname(preprocessings_folder), 'initialized_model.xml')
    doc = parseString((et.tostring(
        model_xml_0).decode('utf-8').replace('\n', '').replace('\t', ''))).toprettyxml()
    np.savetxt(model_xml_path, [doc], fmt='%s')
    ...
    # Clean folder.
    longitudinal_atlas_output_path = join(
    ...
    # Template.
    for k, (object_name, obj_extension) in enumerate(zip(global_objects_name,
                                                                 global_objects_name_extension)):
        estimated_template_path = join(
            ...
            (object_name,
             ...
             model.get_reference_time(), obj_extension))
        global_initial_objects_template_path[k] = join(
            global_path_to_data,
            'ForInitialization__Template_%s__FromLongitudinalAtlas%s' % (object_name, obj_extension))
        shutil.copyfile(estimated_template_path, global_initial_objects_template_path[k])
    ...
    model_xml_0 = insert_model_xml_template(
        model_xml_0, 'filename', global_initial_objects_template_path)

    # Control points.
    estimated_cp_path = join(
        longitudinal_atlas_output_path, 'LongitudinalAtlas__EstimatedParameters__ControlPoints.txt')
    global_initial_cp_path = join(
        global_path_to_data, 'ForInitialization__ControlPoints__FromLongitudinalAtlas.txt')
    shutil.copyfile(estimated_cp_path, global_initial_cp_path)
    model_xml_0 = insert_model_xml_level1_entry(
        model_xml_0, 'initial-control-points', global_initial_cp_path)

    # Momenta.
    estimated_momenta_path = join(longitudinal_atlas_output_path, 'LongitudinalAtlas__EstimatedParameters__Momenta.txt')
    global_initial_momenta_path = join(global_path_to_data, 'ForInitialization__Momenta__FromLongitudinalAtlas.txt')
    shutil.copyfile(estimated_momenta_path, global_initial_momenta_path)
    model_xml_0 = insert_model_xml_level1_entry(model_xml_0, 'initial-momenta', global_initial_momenta_path)

    # Modulation matrix.
    estimated_modulation_matrix_path = join(longitudinal_atlas_output_path, 'LongitudinalAtlas__EstimatedParameters__ModulationMatrix.txt')
    global_initial_mod_matrix_path = join(global_path_to_data, 'ForInitialization__ModulationMatrix__FromLongitudinalAtlas.txt')
    shutil.copyfile(estimated_modulation_matrix_path, global_initial_mod_matrix_path)
    model_xml_0 = insert_model_xml_level1_entry(model_xml_0, 'initial-modulation-matrix', global_initial_mod_matrix_path)

    # Reference time.
    estimated_reference_time_path = join(
    ...
    model_xml_0 = insert_model_xml_deformation_parameters(model_xml_0, 't0', '%.4f' % global_initial_reference_time)

    # Time-shift variance.
    estimated_time_shift_std_path = join(
    ...
    model_xml_0 = insert_model_xml_level1_entry(
        model_xml_0, 'initial-time-shift-std', '%.4f' % global_initial_time_shift_std)

    # Acceleration variance.
    estimated_acceleration_std_path = join(
    ...
    model_xml_0 = insert_model_xml_level1_entry(
        model_xml_0, 'initial-acceleration-std', '%.4f' % global_initial_acceleration_std)
    ...
    model_xml_0 = insert_model_xml_template(model_xml_0, 'noise-std', global_initial_noise_std_string)

    # Onset ages.
    estimated_onset_ages_path = join(longitudinal_atlas_output_path,
                                             'LongitudinalAtlas__EstimatedParameters__OnsetAges.txt')
    global_initial_onset_ages_path = join(global_path_to_data, 'ForInitialization__OnsetAges__FromLongitudinalAtlas.txt')
    shutil.copyfile(estimated_onset_ages_path, global_initial_onset_ages_path)
    model_xml_0 = insert_model_xml_level1_entry(model_xml_0, 'initial-onset-ages', global_initial_onset_ages_path)

    # Accelerations.
    estimated_accelerations_path = join(longitudinal_atlas_output_path, 'LongitudinalAtlas__EstimatedParameters__Accelerations.txt')
    global_initial_accelerations_path = join(
    ...
    # Sources.
    estimated_sources_path = join(longitudinal_atlas_output_path,
                                          'LongitudinalAtlas__EstimatedParameters__Sources.txt')
    global_initial_sources_path = join(global_path_to_data, 'ForInitialization__Sources__FromLongitudinalAtlas.txt')
    shutil.copyfile(estimated_sources_path, global_initial_sources_path)
    model_xml_0 = insert_model_xml_level1_entry(
        model_xml_0, 'initial-sources', global_initial_sources_path)
    ...
    model_xml_path = join(os.path.dirname(preprocessings_folder), 'initialized_model.xml')
    doc = parseString(
        (et.tostring(model_xml_0).decode('utf-8').replace('\n', '').replace('\t', ''))).toprettyxml()
    np.savetxt(model_xml_path, [doc], fmt='%s')
...
def initialize_longitudinal_atlas(model_xml_path, dataset_xml_path, optimization_parameters_xml_path,
    ...
    if not global_overwrite and os.path.isdir(atlas_output_path):
        ...
        for object_id, object_specs in xml_parameters.template_specifications.items():
            ...
            filename = join(global_path_to_data, 'ForInitialization__Template_%s__FromAtlas%s' % (object_id, extension))
        ...
        # Adapt the global settings, for the custom output directory.
        global_deformetrica.output_dir = atlas_output_path
        # Settings().state_file = join(Settings().output_dir, 'pydef_state.p')
        ...
        for k, (object_name, obj_extension, original_obj_noise_var) \
            ...
            estimated_template_path = join(
                atlas_output_path,
                atlas_type + 'Atlas__EstimatedParameters__Template_' + object_name + obj_extension)
            global_initial_objects_template_path.append(join(
                global_path_to_data, 'ForInitialization__Template_' + object_name + '__FromAtlas' + obj_extension))
            shutil.copyfile(estimated_template_path, global_initial_objects_template_path[k])
        ...
        if global_initial_cp_are_given:
            ...
            estimated_cp_path = join(atlas_output_path,
        ...
        # Modify and write the model.xml file accordingly.
        model_xml_0 = et.parse(model_xml_path).getroot()
        model_xml_0 = insert_model_xml_template(model_xml_0,
                                                                'filename', global_initial_objects_template_path)
        model_xml_0 = insert_model_xml_template(model_xml_0,
                                                                'noise-std', global_objects_noise_std_string)
        model_xml_0 = insert_model_xml_level1_entry(model_xml_0,
                                                         'initial-control-points', global_initial_cp_path)
        model_xml_path = join(os.path.dirname(preprocessings_folder), 'initialized_model.xml')
        doc = parseString((et.tostring(
            model_xml_0).decode('utf-8').replace('\n', '').replace('\t', ''))).toprettyxml()
        np.savetxt(model_xml_path, [doc], fmt='%s')
    ...
    # Check if an initial (longitudinal) momenta is available.
        ...
        # Warning.
        if not global_initial_cp_are_given and global_initial_momenta_are_given:
        ...
        for i in range(global_nb_of_subjects):
            ...
            # Set the initial template as the reconstructed object after the atlas.
            for k, (object_id, object_specs) in enumerate(xml_parameters.template_specifications.items()):
                object_specs['filename'] = join(
                    atlas_output_path,
                    '%sAtlas__Reconstruction__%s__subject_%s%s' %
                    (atlas_type, object_id, global_subject_ids[i], global_objects_name_extension[k]))

            # Find the control points and momenta that transforms the previously computed template into the individual.
            registration_cp, registration_momenta = shoot(
                global_initial_cp, global_atlas_momenta[i],
                global_kernel_width, global_kernel_type,
                number_of_time_points=global_nb_of_tp,
                dense_mode=global_dense_mode, tensor_scalar_type=global_tensor_scalar_type)

            # Dump those control points, and use them for the regression.
            path_to_regression_cp = join(
                regression_tmp_path, 'regression_cp__%s.txt' % global_subject_ids[i])
            np.savetxt(path_to_regression_cp, registration_cp)
            xml_parameters.initial_control_points = path_to_regression_cp
            ...
            # Parallel transport of the estimated momenta.
            transported_regression_cp, transported_regression_momenta = parallel_transport(
                regression_cp, regression_momenta, - registration_momenta, #source_cp, source_mom, driving_mom
                global_kernel_width, global_kernel_type, global_kernel_device,
                number_of_time_points=global_nb_of_tp,
                dense_mode=global_dense_mode, tensor_scalar_type=global_tensor_scalar_type)
            ...
            # Saving this transported momenta.
            sub_transported_regression_momenta_path = join(
                regressions_output_path, 'GeodesicRegression__subject_' + global_subject_ids[i],
                'GeodesicRegression__EstimatedParameters__TransportedMomenta.txt')
            np.savetxt(sub_transported_regression_momenta_path, transported_regression_momenta)

        # Divide to obtain the average momenta. Write the result in the data folder.
        global_initial_momenta /= float(global_nb_of_subjects)
        global_initial_momenta_path = join(global_path_to_data, 'ForInitialization__Momenta__FromRegressions.txt')
        np.savetxt(global_initial_momenta_path, global_initial_momenta)
    ...
    global_initial_cp_torch = torch.from_numpy(
        global_initial_cp).type(global_tensor_scalar_type)
    ...
    for i in range(global_nb_of_subjects):
        ...
        sub_transported_regression_momenta_path = join(
            regressions_output_path, 'GeodesicRegression__subject_' + global_subject_ids[i],
            'GeodesicRegression__EstimatedParameters__TransportedMomenta.txt')
        subject_regression_momenta = read_3D_array(sub_transported_regression_momenta_path)
        regression_momenta_torch = torch.from_numpy(
        ...
        if momenta_sc_product_with_pop_momenta <= 0.0:
            msg = 'Subject %s seems to evolve against the population: scalar_product = %.3E.' % \
                  (global_subject_ids[i],
                   Decimal(float(momenta_sc_product_with_pop_momenta)))
            warnings.warn(msg)
            logger.info('>> ' + msg)
            heuristic_accelerations.append(1.0)  # Neutral initialization.
        else:
            heuristic_accelerations.append(
                float(np.sqrt(momenta_sc_product_with_pop_momenta
    ...
    # Standard deviations.
    heuristic_time_shift_std = np.std(heuristic_onset_ages)
    heuristic_acceleration_std = get_acceleration_std(heuristic_accelerations)

    # Rescaling the initial momenta according to the mean of the acceleration factors.
    expected_mean_acceleration = float(truncnorm.stats(- 1.0 / heuristic_acceleration_std, float('inf'),
                                                       loc=1.0, scale=heuristic_acceleration_std, moments='m'))
    mean_acceleration = np.mean(heuristic_accelerations)
    heuristic_accelerations *= expected_mean_acceleration / mean_acceleration
    global_initial_momenta *= mean_acceleration / expected_mean_acceleration
    ...
    model_xml_0 = et.parse(model_xml_path).getroot()
    model_xml_0 = insert_model_xml_level1_entry(
        model_xml_0, 'initial-momenta', global_initial_momenta_path)
    if heuristic_time_shift_std > 0.0:
        model_xml_0 = insert_model_xml_level1_entry(
            model_xml_0, 'initial-time-shift-std', '%.4f' % heuristic_time_shift_std)
    if heuristic_acceleration_std > 0.0:
        model_xml_0 = insert_model_xml_level1_entry(
            model_xml_0, 'initial-acceleration-std', '%.4f' % heuristic_acceleration_std)
    model_xml_0 = insert_model_xml_level1_entry(
        model_xml_0, 'initial-onset-ages', heuristic_onset_ages_path)
    model_xml_0 = insert_model_xml_level1_entry(
        model_xml_0, 'initial-accelerations', heuristic_accelerations_path)
    ...
    if not (global_initial_cp_are_given and global_initial_mod_matrix_is_given):
        ...
        # Load.
        control_points = read_2D_array(xml_parameters.initial_control_points)
        momenta = read_3D_array(join(atlas_output_path, atlas_type + 'Atlas__EstimatedParameters__Momenta.txt'))
        ...
        # Save.
        global_initial_mod_matrix_path = \
            join(global_path_to_data, 'ForInitialization__ModulationMatrix__FromICA.txt')
        np.savetxt(global_initial_mod_matrix_path, global_initial_mod_matrix)
        ...
        model_xml_0 = et.parse(model_xml_path).getroot()
        model_xml_0 = insert_model_xml_level1_entry(
            model_xml_0, 'initial-modulation-matrix', global_initial_mod_matrix_path)
        model_xml_0 = insert_model_xml_level1_entry(
            model_xml_0, 'initial-sources', global_initial_sources_path)
        model_xml_path = join(os.path.dirname(preprocessings_folder), 'initialized_model.xml')
        doc = parseString(
    ...
    if os.path.isdir(registration_output_path):
        ...
        elif not os.path.isdir(join(registration_output_path, 'tmp')):
            registrations = os.listdir(registration_output_path)
            if len(registrations) > 0:
                shutil.rmtree(join(registration_output_path, os.listdir(registration_output_path)[-1]))
    ...
    # Sources.
    global_initial_sources_path = join(
    ...
    if global_time_shift_std > 0:
        model_xml_0 = insert_model_xml_level1_entry(
            model_xml_0, 'initial-time-shift-std', '%.4f' % global_time_shift_std)
    if global_acceleration_std > 0:
        model_xml_0 = insert_model_xml_level1_entry(
            model_xml_0, 'initial-acceleration-std', '%.4f' % global_acceleration_std)
    model_xml_0 = insert_model_xml_level1_entry(
        model_xml_0, 'initial-onset-ages', global_initial_onset_ages_path)
    model_xml_0 = insert_model_xml_level1_entry(
        model_xml_0, 'initial-accelerations', global_initial_accelerations_path)
    ...
    # Adapt the global settings, for the custom output directory.
    global_deformetrica.output_dir = longitudinal_atlas_output_path
    # Settings().state_file = join(longitudinal_atlas_output_path, 'pydef_state.p')
    # xml_parameters._further_initialization(global_deformetrica.output_dir)
    ...
    estimated_reference_time_path = join(
        longitudinal_atlas_output_path, 'LongitudinalAtlas__EstimatedParameters__ReferenceTime.txt')
    global_initial_reference_time = np.loadtxt(estimated_reference_time_path)
    model_xml_0 = insert_model_xml_deformation_parameters(
        model_xml_0, 't0', '%.4f' % global_initial_reference_time)
    ...
    global_initial_noise_std_string = ['{:.4f}'.format(math.sqrt(elt)) for elt in global_initial_noise_variance]
    model_xml_0 = insert_model_xml_template(
        model_xml_0, 'noise-std', global_initial_noise_std_string)
