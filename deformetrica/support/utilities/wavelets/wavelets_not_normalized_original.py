...
class WaveletTransform:
    ...
    def _haar_forward_step_1d(self, wc_cur, dim: int): #wc_cur : part of wc that will be wavelet trasnformed
        #print("_haar_forward_step_1d")
        wc_swap = wc_cur.swapaxes(dim, 0) #current dim as first axis
        wc_lowcur = wc_swap
        
        n = wc_lowcur.shape[0]
        n_low = np.ceil(n/2).astype('int') #highest integer

        if (n == 1):
            wc_comb = wc_lowcur
        else:
            if (n % 2 == 0): #as much pair as impair rows 
                #sum pair and impair rows -> fuse 2 consecutive rows -> mean of 2 rows
                wc_low = (wc_lowcur[0::2] + wc_lowcur[1::2]) / 2 #select x[0], x[2], x[4]... and x[1], x[3], x[5]
            else:
                #same and concatenate with last row
    ...
    def _haar_forward_step(self):
        #print("_haar_forward_step")
        wc_lowcur = self.wc[tuple([slice(None, s[0]) for s in self.ws])]
        #print("wc_lowcur", wc_lowcur)
        for i,dd in enumerate(self.wc.shape): #for each dimension in X
