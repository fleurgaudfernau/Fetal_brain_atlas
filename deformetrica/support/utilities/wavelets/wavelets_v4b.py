...
class WaveletTransform:
    ...
    def _haar_backward_step(self, j): #add j
        wc_cur = self.wc[tuple([slice(None, s[1]) for s in self.ws])]
        for i,dd in enumerate(self.wc.shape):
            self._haar_backward_step_1d(wc_cur, i, j) #add j
    ...
    def haar_backward_inplace(self):
        ...
        for j in range(self.J):
            self._haar_backward_step(self.J-j-1)#self.J-j-1
        self.J = 0
        return(self)
...
def haar_backward_transpose(X, J = None, gamma = 1):
    haar = WaveletTransform()
    haar.haar_forward(X, J, gamma) #haar_forward_dummy
