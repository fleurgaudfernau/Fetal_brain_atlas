...
def haar_filter_HF(haar, current_scale, coef_to_silence = dict()):
    indices_to_browse_along_dim = [list(range(e)) for e in list(haar.wc.shape)]

    for indices in itertools.product(*indices_to_browse_along_dim):
        # we browse all (x,y) positions in 2D grid
        _, _, scale = haar.haar_coeff_pos_type([i for i in indices])

        #silence finer zones that we haven't reached yet
        if scale < current_scale: #the higher the scale, the coarser
            haar.wc[indices] = 0 #fine scales : only ad, da, dd
    
    return haar
        

def get_max_scale(array):
    return np.ceil(np.log2(np.max(array.shape))).astype('int')
...
def array_to_right_dim(X, points_per_axis):
    X = X.reshape(tuple(points_per_axis))

def haar_backward_transpose(X, J = None, gamma = 1, points_per_axis = None):
    haar = WaveletTransform()
    haar.haar_forward(X, J, gamma)
